...
export default async function AboutPage() {
  ...
  // 실시간 수치 조회
  const [{ count: templateCount }, { count: orderCount }, { data: dlData }] = await Promise.all([
    supabaseServer.from('templates').select('*', { count: 'exact', head: true }).eq('is_published', true),
    supabaseServer.from('orders').select('*', { count: 'exact', head: true }).eq('status', 'completed'),
    supabaseServer.from('templates').select('download_count'),
  ])

  const totalDownloads = (dlData ?? []).reduce((s: number, t: any) => s + (t.download_count ?? 0), 0)

  const stats = [
    { n: `${templateCount ?? 0}+`,                    l: '프리미엄 템플릿' },
    { n: `${totalDownloads.toLocaleString()}+`,        l: '다운로드'        },
    { n: '4.9★',                                      l: '평균 평점'       },
    { n: `${(orderCount ?? 0).toLocaleString()}+`,     l: '누적 구매'       },
  ]
          ...
          {[
            ...
            <div key={label} className="flex items-center gap-3 bg-white/[0.03] border border-white/[0.06] rounded-xl p-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-sand/30 mt-0.5">{sub}</p>
              </div>
            </div>
          ))}
        </div>
