...
import { toggleWish as toggleWishDB, getWishedIds } from '@/lib/wishlist'
...
export default function TemplatesPage() {
  ...
  useEffect(() => {
    if (!userId) return
    getWishedIds(userId).then(setWished)
  }, [userId])
  ...
  useEffect(() => {
    ...
    filterTemplates(
      ...
      const sorted = [...data].sort((a, b) => {
        ...
        return b.download_count - a.download_count  // popular (기본)
      })
      setFiltered(sorted)
      setLoading(false)
    })
  }, [query, category, stacks, maxPrice, sort])
                ...
                <div key={t.id} className="relative group">
                  ...
                  <button
                    ...
                    className={`absolute top-2 left-2 z-10 w-7 h-7 rounded-full
                               ...
                               ${wished.has(t.id) ? '!opacity-100 text-[#ff5f3f]' : 'text-sand/40'}`}
                  >
                    ♥
                  </button>
